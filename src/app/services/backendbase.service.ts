...
import { HttpBackend, HttpHeaders, HttpParams, HttpResponse, HttpRequest, HttpClient } from "@angular/common/http";
...
export class BackendBaseService {
  ...
  protected getApi(url, params: any = null, config: any = null) {
    ...
    return this.http.get(url, { headers: headers, params: query, responseType: config });
  }

  protected getApiToSearch(url, params: any = null, config: any = null) {
    let headers = new HttpHeaders().set('Content-Type', 'application/json').set('Accept', 'application/json');
    let query = this.loopParams(params);
    return this.http.get(url, { headers: headers, params: query, responseType: config }).pipe(
      tap((response: any) => {
        return response;
      })
    );
