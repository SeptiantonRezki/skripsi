...
@Component({
  selector: 'app-import-keyword',
  templateUrl: './import-keyword.component.html',
  styleUrls: ['./import-keyword.component.scss']
})
export class ImportKeyword implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<ImportKeyword>,
  ...
  preview(event) {
    ...
    this.keywordService.importExcel(fd).subscribe(
      res => {
        if (res && res.data) {
  ...
  recursiveImport() {
    if (this.currPage <= this.lastPage) {
      this.keywordService.showImport({ page: this.currPage, per_page: this.pagination['per_page'] }).subscribe(response => {
        // console.log('dua');
  ...
  submit() {
    ...
    if(IMPORT_TYPE === 'AUDIENCE') {
      ...
      if (this.totalData > 0) {
        this.dialogRef.close({importID: this.importID});
      } else {
  ...
  trialImport() {
    ...
    this.trials.map(trial => {
      let response = this.keywordService.showImport({ page: trial, per_page: this.pagination['per_page'] });
      // console.log('tiga');
