...
@Component({
  selector: 'app-virtual-account-panel-src-edit',
  templateUrl: './virtual-account-panel-src-edit.component.html',
  styleUrls: ['./virtual-account-panel-src-edit.component.scss']
})
export class VirtualAccountPanelSrcEditComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.VirtualAccountCompanyId = this.dataService.getFromStorage('company_selected') || null;
    ...
    this.shortDetail = this.dataService.getFromStorage("detail_virtual_account_panel");
  }
  ...
  //       company: res.data.virtual_account_company_id
  //     });
  //     this.aturPanelMitra();
  //   }, err => {
  ...
  aturPanelMitra() {
    if (this.VirtualAccountCompanyId !== null) {
      this.selectedMitra = [];
      ...
      this.dataService.showLoading(true);
      this.panelService.checkPanel({ virtual_account_company_id: this.VirtualAccountCompanyId }).subscribe(res => {
        // console.log('res', res);
        // this.getPanelSrcList();
        // console.log('res', res);
  ...
  async export() {
    ...
    try {
      ...
      this.downLoadFile(response, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", `Export_VirtualAccountPanelSrc_${new Date().toLocaleString()}.xls`);
