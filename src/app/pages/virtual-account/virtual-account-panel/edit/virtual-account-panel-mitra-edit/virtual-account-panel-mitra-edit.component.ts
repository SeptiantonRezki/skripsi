...
@Component({
  selector: 'app-virtual-account-panel-mitra-edit',
  templateUrl: './virtual-account-panel-mitra-edit.component.html',
  styleUrls: ['./virtual-account-panel-mitra-edit.component.scss']
})
export class VirtualAccountPanelMitraEditComponent implements OnInit, OnDestroy {
  ...
  getDetail() {
    ...
    this.mitraPanelService.show({ panel_id: this.shortDetail.id }).subscribe(res => {
  ...
  aturPanelMitra() {
    if (this.formPanelMitra.valid) {
      ...
      this.dataService.showLoading(true);
      this.mitraPanelService.checkPanel({ virtual_account_company_id: this.formPanelMitra.get('company').value }).subscribe(res => {
        console.log('res', res);
        // this.getPanelMitraList();
        // console.log('res', res);
        let filteredMitra = [];
  ...
  async export() {
    ...
    try {
      ...
      this.downLoadFile(response, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", `Export_VirtualAccountPanelMitra_${new Date().toLocaleString()}.xls`);
