...
@Component({
  selector: 'app-coin-adjustment-approval-detail',
  templateUrl: './coin-adjustment-approval-detail.component.html',
  styleUrls: ['./coin-adjustment-approval-detail.component.scss']
})
export class CoinAdjustmentApprovalDetailComponent implements OnInit {
  dataApproval: any = {};
  ...
  getListAudience(id: any, search?: string) {
    ...
    this.coinAdjustmentApprovalService.getListAudience({ audience_id: id, is_tsm: this.isTSM }, this.pagination).subscribe(res => {
  ...
  setPage(pageInfo) {
    ...
    this.coinAdjustmentApprovalService.getListAudience({ audience_id: this.trade_audience_group_id }).subscribe(res => {
      Page.renderPagination(this.pagination, res.data);
      this.rows = res.data.data;
      this.loadingIndicator = false;
      this.dataService.showLoading(false);
