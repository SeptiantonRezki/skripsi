...
export class TaskVerificationDetailComponent implements OnInit {
  ...
  import() {
    ...
    if (this.dataScheduler.status_berjalan === 'expired') {
      return this.dialogService.openSnackBar({
        message: `${this.translate.instant('dte.task_verification.import_expired_message')} ${this.dataScheduler.status_berjalan}`
      });
    }
    ...
    this.dialogRef = this.dialog.open(ImportCoinComponent, dialogConfig); // TODO
  ...
  openConfirmDialog(item: any, popupType: string) {
    ...
    this.dialogRef = this.dialog.open(ConfirmDialogComponent, dialogConfig); // TODO
