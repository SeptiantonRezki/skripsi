...
@Component({
  ...
  templateUrl: './coin-disburstment-exchange.component.html',
  styleUrls: ['./coin-disburstment-exchange.component.scss']
})
export class CoinDisburstmentExchangeComponent implements OnInit, OnDestroy {
  onLoad: boolean;
  ...
  setPage(pageInfo) {
    ...
    this.coinDisburstmentService.getExhanges(this.pagination).subscribe(res => {
      Page.renderPagination(this.pagination, res);
      this.rows = res.data;
      this.loadingIndicator = false;
    });
  ...
  async exportExchange(isDetail: boolean) {
    ...
    try {
      ...
      link.download = `list-${isDetail ? "detail-" : ""}penukaran-coin-${getTime}.xlsx`;
