...
@Component({
  selector: 'app-panel-mitra-coin',
  templateUrl: './panel-mitra-coin.component.html',
  styleUrls: ['./panel-mitra-coin.component.scss']
})
export class PanelMitraCoinComponent implements OnInit {
  ...
  getListMitra(string?: any) {
    ...
    try {
      ...
      this.shopingDiscountCoinsService.getMitra(this.pagination, body).subscribe(res => {
  ...
  importMitra(): void {
    if (this.statusCoinDiscount === 'need-approval' && this.permissions['b2b_approval']) {
    ...
    dialogConfig.data = { discount_id: this.detailCoinDiscount.id, type: 'wholesaler' };
