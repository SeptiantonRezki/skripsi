...
export class ImportAudiencePersonalizeInfoBoardComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<ImportAudiencePersonalizeInfoBoardComponent>,
  ...
  preview(event) {
    ...
    this.infoBoardService.importExcel(fd).subscribe(
      res => {
        if (res && res.data) {
          this.dataService.showLoading(false);
          this.pagination['per_page'] = 1000;
          this.infoBoardService.showImport(this.pagination).subscribe(response => {
  ...
  recursiveImport() {
    if (this.currPage <= this.lastPage) {
      this.infoBoardService.showImport({ page: this.currPage, per_page: this.pagination['per_page'] }).subscribe(response => {
        // console.log('dua');
  ...
  trialImport() {
    ...
    this.trials.map(trial => {
      let response = this.infoBoardService.showImport({ page: trial, per_page: this.pagination['per_page'] });
      // console.log('tiga');
