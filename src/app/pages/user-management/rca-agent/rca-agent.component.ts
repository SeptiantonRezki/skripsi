...
@Component({
  selector: 'app-rca-agent',
  templateUrl: './rca-agent.component.html',
  styleUrls: ['./rca-agent.component.scss']
})
export class RcaAgentComponent implements OnInit {
  rows: any[];
  selected: any[];
  id: any[];
  ...
  onSort(event) {
    ...
    this.rcaAgentService.getList(this.pagination).subscribe(res => {
      Page.renderPagination(this.pagination, res);
      this.rows = res.data;
      this.loadingIndicator = false;
    });
