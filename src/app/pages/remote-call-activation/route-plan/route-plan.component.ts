...
export class RoutePlanComponent implements OnInit {
  ...
  getListRoutePlan(search?: string) {
    ...
    if (this.pagination['position']) this.positionCodeSummary.setValue(this.pagination['position']);

    console.log('value', this.positionCodeSummary.value);
  ...
  exportGrouping() {
    ...
    this.rcaAgentService.exportRoutePlan({ area: this.pagination['area'], ...params }).subscribe(res => {
      console.log('res', res);
